...
class CAFTTA(nn.Module):
    ...
    def forward_and_adapt(self, x, model, model0, optimizer):
        ...
        filter_idx = torch.where(score_reverse < self.hyperparameters[0] * math.log(1000), 0, 1) 
        filter_idx2 = torch.where(score_reverse > self.hyperparameters[1] * math.log(1000), 0, 1) 
