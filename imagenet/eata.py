...
class EATA(nn.Module):
    ...
    def __init__(self, model, optimizer, fishers=None, fisher_alpha=2000.0, steps=1, episodic=False, 
                e_margin=math.log(1000)/2-1, d_margin=0.05, alpha=[0.5], criterion="ent"):
        super().__init__()
        self.model = model
    ...
    def forward(self, x):
        ...
        if self.steps > 0:
            for _ in range(self.steps):
                outputs, num_counts_2, num_counts_1, updated_probs = forward_and_adapt_eata(x, self.model0, self.model, self.optimizer, self.fishers, 
                                                                                            self.e_margin, self.current_model_probs, fisher_alpha=self.fisher_alpha, 
...
def forward_and_adapt_eata(x, model0, model, optimizer, fishers, e_margin, current_model_probs, fisher_alpha=50.0, d_margin=0.05, 
                           num_samples_update=0, alpha=[0.5], criterion="ent"):
    ...
    if criterion != "ent":
        ...
        if current_model_probs is not None: 
            cosine_similarities = F.cosine_similarity(current_model_probs.unsqueeze(dim=0), outputs[filter_ids_1].softmax(1), dim=1)
            filter_ids_2 = torch.where(torch.abs(cosine_similarities) < d_margin)
            # print(len(filter_ids_1[0]), len(filter_ids_2[0]))
...
def update_model_probs(current_model_probs, new_probs):
    if current_model_probs is None:
        ...
        if new_probs.size(0) == 0:
            with torch.no_grad(): return current_model_probs
        else:
            with torch.no_grad(): return 0.9 * current_model_probs + (1 - 0.9) * new_probs.mean(0)
