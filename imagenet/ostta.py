...
class OSTTA(nn.Module):
    ...
    def __init__(self, model, optimizer, steps=1, episodic=False, alpha=[0.5], criterion="ent"):
        ...
        for param in self.model0.parameters(): param.detach()
    ...
    def forward(self, x):
        ...
        for _ in range(self.steps):
            outputs = forward_and_adapt(x, self.model0, self.model, self.optimizer, self.alpha, self.criterion, self)
...
def forward_and_adapt(x, model0, model, optimizer, alpha, criterion, ostta):
    ...
    with torch.no_grad():
        if criterion != "ent":
            ...
            cos_sim = F.cosine_similarity(model1(x).unsqueeze(1).to('cpu'), model.model.fc.weight.to('cpu'), dim=2)
    ...
    if criterion == 'ent':
        condition = values >= values0

        ostta._inlier_sum += sum(condition[:32])
        ostta._outlier_sum += sum(condition[32:])
    # logging
    elif criterion == 'ent_ind_ood':
        try:
            # 1. filter_ids == 0 인 인덱스 찾기
            filtered_indices = (filter_ids == 0).nonzero()[0]

            # 2. values와 values0의 비교 조건에 맞는 인덱스 찾기
            condition = values[filtered_indices] >= values0[filtered_indices]
            selected_indices = filtered_indices[condition.to('cpu')]

            ostta._inlier_sum += sum(selected_indices < 32)
            ostta._outlier_sum += sum(selected_indices >= 32)
        except: pass
