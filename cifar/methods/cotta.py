...
class CoTTA(nn.Module):
    ...
    """
    def __init__(self, model, optimizer, steps=1, episodic=False, mt_alpha=0.99, rst_m=0.1, ap=0.9, criterion='ent'):
        super().__init__()
        self.model = model
    ...
    def forward_and_adapt(self, x, model, optimizer):
        ...
        N = 32 
        outputs_emas = []
        if self.criterion != "ent":
            model1 = deepcopy(self.model_anchor)
        ...
        if anchor_prob.mean(0) < self.ap: outputs_ema = torch.stack(outputs_emas).mean(0)
        else: outputs_ema = standard_ema

        # Student update (UniEnt)
        if self.criterion != "ent":  
            if self.criterion != "ent_unf":
                loss = softmax_entropy_ema(outputs, outputs_ema)[filter_ids == 0].mean(0)
                
            if self.criterion != "ent_ind":
                entropys_ood = softmax_entropy(outputs)[filter_ids == 1]
                loss -= self.alpha[1] * entropys_ood.mean(0)
            loss -= self.alpha[0] * softmax_mean_entropy(outputs)
        else:  # criterion == ent
...
def softmax_entropy_ema(x, x_ema): return -(x_ema.softmax(1) * x.log_softmax(1)).sum(1)
