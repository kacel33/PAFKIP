...
class EATA(nn.Module):
    ...
    """
    def __init__(self, model, optimizer, fishers=None, fisher_alpha=1.0, steps=1, episodic=False, e_margin=math.log(1000)/2-1, d_margin=0.05, alpha=[0.5], criterion="ent"):
        super().__init__()
        self.model = model
...
def update_model_probs(current_model_probs, new_probs):
    if current_model_probs is None:
        if new_probs.size(0) == 0: return None
        else:
            with torch.no_grad(): return new_probs.mean(0)
    else:
