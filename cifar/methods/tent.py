...
class Tent(nn.Module):
    ...
    def __init__(self, model, optimizer, steps=1, episodic=False, alpha=[0.5], criterion="ent"):
        ...
        self._model_caftta = deepcopy(self.model)
        for param in self._model_caftta.parameters():
            param.detach()
        self._model_caftta.eval()
...
def forward_and_adapt(x, model0, model, optimizer, alpha, criterion, tent):
    ...
    if criterion != "ent":
        ...
        if criterion != "ent_unf":
            ...
            tent._inlier_sum += 100 - filter_ids[:100].sum()
            tent._outlier_sum += 100 - filter_ids[100:].sum()
        else:
            filter_ids = gm.predict(os.detach().cpu().numpy().reshape(-1, 1))
            filter_ids = filter_ids if gm.means_[0, 0] < gm.means_[1, 0] else 1 - filter_ids
...
def update_fixed_model_block(fixed_model, train_model, block_name):
    train_dict = getattr(train_model, block_name).state_dict()
    fixed_dict = getattr(fixed_model, block_name).state_dict()
    updated_dict = {k: train_dict[k] for k in train_dict if k in fixed_dict}
    fixed_dict.update(updated_dict)
    getattr(fixed_model, block_name).load_state_dict(fixed_dict)
