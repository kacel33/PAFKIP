...
class SoTTA(nn.Module):
    def __init__(self, model, optimizer, memory_size, threshold, num_class, steps=1, episodic=False):
        super().__init__()
        self.model = model
...
def forward_and_adapt(x, model, memory, optimizer):
    ...
    for i in range(x.shape[0]): memory.add_instance([x[i].detach(), pseudo_labels[i], confidences[i]])
