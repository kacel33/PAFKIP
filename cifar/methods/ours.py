...
class PAFKIP(nn.Module):
    ...
    def forward_and_adapt(self, x, optimizer):
        ...
        with torch.no_grad(): caftta_outputs = self.model0(x)
        ...
        if len(loss_ind_weighted) > 0: loss = loss + loss_ind_weighted.mean(0)
...
def get_coeff(ent_list, ent_thr_ratio, n_class):
    # BF16 안전화: exp 입력 clamp로 오버플로우/언더플로우 방지
